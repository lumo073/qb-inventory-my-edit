...
--#endregion Variables
QBCore.Functions.CreateCallback('DoesPlayerHaveDesiredPermission', function(source, cb)
	local player = QBCore.Functions.GetPlayer(source)
	local permissionsToCheck = {"god", "admin",} -- you can change the name of the perms to ones you are using also you can use multiple or just one permission you are not restriced in any way 
	local hasPermission = QBCore.Functions.HasPermission(source, permissionsToCheck) -- you can use the permissions table or specify one permission group so if you're not using the table permissionsToCheck (there's no reason why not but ok :neutral_face:) then instead of (source, permissionsToCheck) write (source, "god") or again any permission name you are using
...
local function OpenInventory(name, id, other, origin)
	...
	if name and id then
		...
		elseif name == "creative" then
			secondInv.name = "creative"
			secondInv.label = other.label
			secondInv.maxweight = other.maxweight
			secondInv.inventory = other.items
			CreativeItems[id] = {}
			CreativeItems[id].items = other.items
			secondInv.slots = other.slots
		...
		elseif name == 'otherplayer' then
			...
			if OtherPlayer then
				...
				if (Player.PlayerData.job.name == 'police' or Player.PlayerData.job.type == 'leo') and Player.PlayerData.job.onduty then
					secondInv.slots = Config.MaxInventorySlots + 200
				else
					secondInv.slots = Config.MaxInventorySlots +200
				end
				Wait(250)
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	elseif fromInventory == "creative" then
        local item = CreativeItems[99].items[fromSlot]
        TriggerClientEvent('QBCore:Notify', src, "You moved x" .. fromAmount .. "of " .. item.name .. "!", "primary")
        Player.Functions.AddItem(item.name, fromAmount, toSlot, item.info)
