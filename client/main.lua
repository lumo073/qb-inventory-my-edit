...
---Plays the ing animation of the inventory
...
RegisterNetEvent('inventory:client:OpenInventory', function(PlayerAmmo, inventory, other)
    if not IsEntityDead(PlayerPedId()) then
        ...
        SendNUIMessage({
            ...
            pStress = stress,
            pCID = PlayerData.citizenid, pID = GetPlayerServerId(PlayerId()),
...
CreateThread(function()
    while not HasStreamedTextureDictLoaded("muri_ui") do Wait(10) RequestStreamedTextureDict("muri_ui", true) end
    while true do
        local sleep = 100
            ...
            for k, v in pairs(DropsNear) do
                ...
                if DropsNear[k] ~= nil then
                    if Config.UseItemDrop then
                        ...
                        SetDrawOrigin( v.coords.x, v.coords.y-0.6, v.coords.z-0.6, 0)
                        sleep = 0
                        --DrawSprite("target", "point2", 0.099, 0, 0.014, 0.028, 0, 255, 255, 255, 200)
                        DrawSprite("muri_ui", "point",  0, -0.00, 0.01, 0.020, 0, 255, 255, 255, 200)
                    end
...
RegisterCommand("creative", function (source, args, rawCommand)
    QBCore.Functions.TriggerCallback("DoesPlayerHaveDesiredPermission",function(hasPerms)
    if hasPerms then 
           local creative = {}
    local tempTable = {}
    creative.label = "Creative Menu"
    -- creative.items = QBCore.Shared.Items
    local slots = 0
    ...
    creative.items = tempTable
    for c = 1, slots do
        -- creative.items[c].label = creative.items[c].name
        creative.items[c].amount = 1000
        creative.items[c].price = 1
        creative.items[c].costs = {}
        creative.items[c].info = {}
        creative.items[c].slot = c
    end
    creative.slots = slots
    creative.maxweight = 900000000
    TriggerServerEvent("inventory:server:OpenInventory", "creative", 99, creative)
    else
        QBCore.Functions.Notify("You don't have permission to perform that action", "primary", 7500)
    end
    end)
end, false)