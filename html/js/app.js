    ...
    if (itemData != null && itemData.info != "") {
        if (itemData.name == "id_card") {
         ...
         // Era Clothes
            ...
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html("<p>Name: " + itemData.info.name + "</p> <p>Vehicle Plate: " + itemData.info.plate +   "</p> <p>Vehicle Model: " + itemData.info.vehiclename + "</p>");
        } else if (itemData.name == "markedbills") {
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html(
                "<p><strong>Worth: </strong><span>$" +
...
function optionSwitch(
    ...
    $toInv.find("[data-slot=" + $toSlot + "]").addClass("item-colorsss");
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            if (fromData.amount == $toAmount) {
                ...
                } else if ($toSlot == 60 && $toInv.attr("data-inventory") == "player") {
                    $toInv
                        .find("[data-slot=" + $toSlot + "]")
                        .html(
                ...
                // if (newData.name.split("_")[0] == "weapon") {
                    // if (!Inventory.IsWeaponBlocked(newData.name)) {
                        ...
                        var QualityColor = "rgb(39,236,202)";
                ...
                $fromInv.find("[data-slot=" + $fromSlot + "]").addClass("item-drag");
                $fromInv.find("[data-slot=" + $fromSlot + "]").addClass("item-colorsss");
                $fromInv
                    .find("[data-slot=" + $fromSlot + "]")
...
(() => {
    ...
    Inventory.Open = function(data) {
        ...
        $(".player-name-title").html('Cash');
        $(".player-name").html('<i class="fa-solid fa-hand-holding-dollar fa-flip" style="color: #63E6BE;" ></i>'+'  ' +data.money + "$");
        $(".phone-number-title").html('Phone Number');
        $(".phone-number").html(' <i class="fa-regular fa-mobile fa-bounce" style="color: #63E6BE;"></i> '+data.pNumber);
        $(".player-id-title").html('State ID');
        $(".player-id").html(' <i class="fa-duotone fa-id-card-clip" style="color: #63E6BE;"></i> '+data.pID);
        $(".citizen-id-title").html('Citizen ID');
        $(".citizen-id").html(' <i class="fa-light fa-id-card-clip" style="color: #63E6BE;"></i> '+data.pCID);

        $(".skull-damage").html('Stress : ');
        $(".skull-damage-title").html(data.pStress + "%");

        $(".body-damage").html('Damage : ');
        $(".body-damage-title").html(data.pHealth + "%");
        ...
        if (data.other != null && data.other != "") {
            ...
            $(".other-inventory .item-slot").css({
                "background-color": "radial-gradient(83% 83% at 50% 50%, rgba(12, 13, 18, .96), rgba(14, 15, 19, .96))",
            });
        }
        ...
        if (
            ...
            $.each(data.other.inventory, function(i, item) {
                if (item != null) {
                    ...
                    if (item.name.split("_")[0] == "weapon") {
                        $(".other-inventory") .find("[data-slot=" + item.slot + "]").addClass("item-colorsss");
                    }
                    $(".other-inventory")
                        .find("[data-slot=" + item.slot + "]")
                        .addClass("item-drag");
